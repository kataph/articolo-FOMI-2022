...
\begin{itemize}
  ...
  \item \textbf{Functions as transformative events} \cite{borgoFormalizationFunctionsOperations2011,borgoCapabilitiesCapacitiesFunctionalities2021, garbaczStandardTaxonomyArtifact2005}. Some authors describe classes of functions as particular classes of occurrents. This also is not appropriate because engineers clearly treat functions and \qquotes{functionings} (that is, occurrences of functions) differently: functions, at the very least, keep existing after any individual functioning. One could, say, reify event classes or speak of `state of affairs' to introduce stable entities, but it would be artificious and we would have to categorize such reifications ontologically. %Additionally,  first, we would have to, say, reify the classes of occurrents in order to even have some  
...
Then, a functional decomposition of an engineering system can be achieved, alternating between methods and functions. Figure \ref{fig:functional-decomposition} shows an implementation of this, to decompose the function of a steady rest of alaser cutting machine.
