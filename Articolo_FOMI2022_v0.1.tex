...
Engineering systems and their components are both difficult to characterize from an ontological point of view. \TODO{Take, for example, any schematic of a given system. The actual system may or may not, at any given time, follow such a schematic. For instance, a component may be missing for some period of time. This can be explained saying that the schematic represent a system the way it should be, not the way it actually is, but, unfortunately, this way we must complicate our reasoning, introducing modality. Additionally, this does not explain how one can recognize the actual system and the schematic as linked to the same type of objects. In this section we argue how an appropriate theory of functions can lessen these problems. <--FC:lungo;argomentabilmente sbagliato o fuori contesto;rimuovere} 
...
\begin{itemize}
  ...
  %% to distribute is a class of perdurants in which a certain flow is broken up. EngFuncðxÞ!PDðxÞ -- quote from borgoFormalizationFunctionsOperations2011
   This also is not appropriate  % because engineers clesarly treat functions and \qquotes{functionings} (that is, occurrences of functions) differently: functions, at the very least, keep existing after any individual functioning.<-- FC: nonsenso: se in FR le funzioni sono transizioni tra stati, allora i loro tipi sono tipi di eventi.
   ...
   One could, say, reify event classes \myComment{or speak of `state of affairs' <-- FC: meglio lasciar stare} to introduce stable entities, but it would be artificious and we would have to solve the ensuing problem of categorizing such reifications ontologically. %Additionally,  first, we would have to, say, reify the classes of occurrents in order to even have some  
